...
class UnitTests {
    ...
    fun testMissingRecords() {
        ...
        try {
            LegacyUtils.constructBatchRequestBody(records, InsertOptions(), LogLevel.ERROR)
    ...
    fun testMissingTableForInsert() {
        ...
        try {
            val x = LegacyUtils.constructBatchRequestBody(records, InsertOptions(), LogLevel.ERROR)
    ...
    fun testDateValidator() {
        ...
        assertFalse(date.isValid("10", "xx"))
        assertFalse(date.isValid("0", "23"))
        assertFalse(date.isValid("13", "23"))
        assertFalse(date.isValid("02", "202222"))
        assertFalse(date.isValid("12", (currentYear - 1).toString()))

        for (month in 1..12) {
            if (month < currentMonth) {
                assertFalse(date.isValid("$month", "$currentYear"))
            } else {
                assertTrue(date.isValid("$month", "$currentYear"))
            }
        }
    }
...
//    }
    ...
    fun testConverQueryString() {
        ...
        assertEquals(
            Utils.convertJSONToQueryString(json),
            "card%5Bexp_month%5D=1&card1%5Bexp_year%5D=2024&type=card&card%5Bnumber%5D=4242&card1%5Bcvc%5D=314"
        )
    }
    ...
    fun testGetBody() {
        ...
        json.put("card", card)
        assertTrue(
            LegacyUtils.getRequestbodyForConnection(json, ContentType.FORMURLENCODED.type).contentType()
                .toString().contains(ContentType.FORMURLENCODED.type)
        )
        assertTrue(
            LegacyUtils.getRequestbodyForConnection(json, ContentType.FORMDATA.type).contentType()
                .toString().contains(ContentType.FORMDATA.type)
        )
        assertTrue(
            LegacyUtils.getRequestbodyForConnection(json, ContentType.APPLICATIONORJSON.type)
                .contentType().toString().contains(ContentType.APPLICATIONORJSON.type)
        )
    }
