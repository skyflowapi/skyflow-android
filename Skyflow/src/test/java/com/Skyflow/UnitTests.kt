    ...
    fun testEmptyTokenForDetokenize() {
        ...
        apiClient.get(revealRecords, object : Callback {
            ...
            override fun onFailure(exception: Any) {
                ...
                Assert.assertEquals(
                    skyflowError.getInternalErrorMessage(), getErrorMessage(exception as JSONObject)
                )
            }
        })
        revealRecords.put("records", "{}")
...
//    }
    ...
    fun testGetBody() {
        ...
        json.put("card", card)
        assertTrue(
            Utils.getRequestbodyForConnection(json, ContentType.FORMURLENCODED.type).contentType()
                .toString().contains(ContentType.FORMURLENCODED.type)
        )
        assertTrue(
            Utils.getRequestbodyForConnection(json, ContentType.FORMDATA.type).contentType()
                .toString().contains(ContentType.FORMDATA.type)
        )
        assertTrue(
            Utils.getRequestbodyForConnection(json, ContentType.APPLICATIONORJSON.type)
