...
enum class Messages(val message: String) {
    ...
    INVALID_VAULT_URL("%s SDK v%s Initialization failed. Invalid client credentials. 'vaultURL' must be begin with 'https://'."),
    EMPTY_VAULT_ID("%s SDK v%s Initialization failed. Invalid credentials. Specify a valid 'vaultID'."),
    EMPTY_VAULT_URL("%s SDK v%s Initialization failed. Invalid credentials. Specify a valid 'vaultURL'."),
    INVALID_BEARER_TOKEN("%s SDK v%s Initialization failed. Invalid client credentials. Specify a valid bearer token."),
    INVALID_TABLE_NAME("%s SDK v%s Validation error. Invalid 'table' key in records at index %s. Specify a value of type string instead."),
    ELEMENT_EMPTY_TABLE_NAME("element with type %s -  Table key cannot be empty."),
    EMPTY_TABLE_KEY("%s SDK v%s Validation error.'table' cannot be empty in records at index %s. Specify a non-empty value for 'table'."),
    EMPTY_COLUMN_KEY("column key cannot be empty"),
    RECORDS_KEY_NOT_FOUND("%s SDK v%s Validation error. Missing 'records' key. Provide a valid 'records' key."),
    ADDITION_RECORDS_KEY_KEY_NOT_FOUND("%s SDK v%s Validation error.'records' key not found in additionalFields. Specify a 'records' key in additionalFields."),
    EMPTY_RECORDS("%s SDK v%s Validation error. 'records' key cannot be empty. Provide a non-empty value instead."),
    ...
    MISSING_TABLE_KEY("%s SDK v%s Validation error.'table' key not found in records at index %s. Specify a valid value for 'table' key."),
    ...
    INVALID_EVENT_TYPE("%s SDK v%s Invalid event type. Specify a valid event type."),
    INVALID_EVENT_LISTENER("%s SDK v%s  Invalid event listener. Please specify a valid event listener."),
    UNKNOWN_ERROR("%s"),
    ...
    ELEMENT_NOT_MOUNTED("%s SDK v%s Collect failed. Make sure all elements are mounted before calling 'collect' on the container."),
    DUPLICATE_COLUMN_FOUND("Duplicate element with %s and %s found in container"),
    DUPLICATE_ELEMENT_FOUND("%s SDK v%s Mount failed. '%s' appeared multiple times in '%s'. Make sure each column in a record is unique."),
    ...
    FAILED_TO_REVEAL("%s SDK v%s Reveal failed. Some errors were encountered."),
    ...
    EMPTY_FIELDS("%s SDK v%s Validation error. Missing 'fields' key in records at index %s. Provide a valid 'fields' key."),
    ...
    INVALID_FORMAT_REGEX("%s SDK v%s Validation error. Invalid value for 'regex' param found for regex in validations array at index %s. Provide a valid value regular expression for regex param."),
    ...
    EMPTY_COLUMN_VALUE("%s SDK v%s Validation error. 'column' key cannot be empty in records at index %s. Specify a non-empty value instead."),
    ...
    INPUT_FORMATTING_NOT_SUPPORTED("%s SDK v%s Mount failed. Format must be a non-empty string. Specify a valid format."),
    INVALID_INPUT_TRANSLATION("%s SDK v%s Mount failed. Translation must be a non-empty object. Specify a valid translation."),
    EMPTY_INPUT_TRANSLATION("translation not passed. Switching to default translation %s"),

    MISMATCH_ELEMENT_COUNT_LAYOUT_SUM("%s SDK v%s Mount failed. Invalid layout array values. Make sure all values in the layout array are positive numbers.")
}
